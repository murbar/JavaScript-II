...
let allCaps = runners.map(r => r.first_name.toUpperCase())
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter(r => r.shirt_size === 'L');
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((tally, runner) => {
    return tally + runner.donation
}, 0);
...
// Problem 1
// We want to recognize donors who contributed $200 or more.
let topDonors = runners
    .filter(r => r.donation >= 200)
    .map(r => {
        return { fullName: `${r.first_name} ${r.last_name}`, donation: r.donation };
    })
    .sort((a, b) => a.donation - b.donation)
    .reverse();
// Top donors in descending order
console.log(topDonors);

// Problem 2
// We want a list of email addresses for the marketing dept.
let emailList = runners.map(r => r.email);
console.log(emailList);

// Problem 3
// We want a list of our runners' companies and how many employees they have in the race.
let companyList = []
runners.forEach(r => {
    const company = r.company_name;
    // is the company in our list?
    indexInList = companyList.findIndex(r => r.company === company);
    if (indexInList === -1) {
        companyList.push({ company: company, participants: 1});
    } else {
        companyList[indexInList].participants += 1;
    }
});
companyList.sort((a, b) => b.participants - a.participants);
